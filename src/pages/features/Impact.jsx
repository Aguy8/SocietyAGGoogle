      ...
      <section data-nav-theme="dark" style={{ padding: '120px 64px 80px', background: '#1A2BFF', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <div style={{ display: 'inline-flex', padding: '8px 16px', background: 'rgba(255,255,255,0.12)', borderRadius: 999, marginBottom: 24 }}>
            <Icon name="chart" size={18} color="#B22842" />
            <span style={{ color: '#1A2BFF', fontWeight: 700, fontSize: 13, marginLeft: 8 }}>VISIBILITÉ & IMPACT</span>
          </div>
          <h1 style={{ fontSize: 88, lineHeight: 0.9, color: 'white', maxWidth: 900 }}>
            Prouvez votre<br /><span style={{ color: '#1A2BFF' }}>impact.</span><br />
            Automatiquement.
          </h1>
      ...
      <section style={{ padding: '80px 64px' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24, marginBottom: 80 }}>
            {[
              { n: 14290, suffix: '', prefix: '', label: 'Bénéficiaires touchés en moyenne', color: '#1A2BFF' },
              { n: 62, suffix: ' M', prefix: '', label: 'FCFA collectés en moyenne/an', color: '#B22842' },
              { n: 10, suffix: ' min', prefix: '', label: 'Pour générer un rapport PDF complet', color: '#1F5D3A' },
            ...
            <Reveal direction="left">
              ...
              <div style={{ marginTop: 32, display: 'grid', gap: 12 }}>
                {['URL personnalisée aguy8.github.io/Society.ci/votre-communaute', 'Chiffres mis à jour en temps réel', 'Témoignages des bénéficiaires', 'Galerie de photos et vidéos'].map((c, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 15, color: '#14110D' }}>
                    <span style={{ color: '#1A2BFF', fontWeight: 800 }}>✓</span> {c}
                  </div>
                ))}
              </div>
            </Reveal>
            <Reveal direction="right">
              <div style={{ padding: 40, background: '#1A2BFF', borderRadius: 28, color: 'white' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 24 }}>
                  {[['14 290', 'bénéficiaires'], ['8', 'projets'], ['62 M', 'FCFA'], ['1 248', 'membres']].map(([v, l], i) => (
                    <div key={i} style={{ padding: 16, background: 'rgba(255,255,255,0.08)', borderRadius: 12, textAlign: 'center' }}>
                      <div style={{ fontSize: 28, fontWeight: 800, color: '#1A2BFF' }}>{v}</div>
                ...
                <div style={{ marginTop: 16, padding: 16, background: 'rgba(255,255,255,0.06)', borderRadius: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {['ODD 1', 'ODD 4', 'ODD 8', 'ODD 17'].map(odd => (
                      <span key={odd} style={{ padding: '4px 10px', background: 'rgba(212,167,91,0.2)', borderRadius: 999, fontSize: 11, fontWeight: 700, color: '#1A2BFF' }}>{odd}</span>
                    ))}
                  </div>
                </div>
                <button style={{ marginTop: 16, width: '100%', padding: 14, background: '#B22842', color: '#14110D', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
      ...
      <section data-nav-theme="dark" style={{ padding: '80px 64px 120px', background: '#14110D' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: 56, color: 'white', fontWeight: 800, maxWidth: 700, margin: '0 auto' }}>
            Votre impact mérite<br />d'être <span style={{ color: '#1A2BFF' }}>visible</span>.
          </h2>
          <NavLink to="/inscription" className="btn btn-gold btn-xl" style={{ marginTop: 40, display: 'inline-flex' }}>
            Créer ma page d'impact <Icon name="arrow" size={16} color="#14110D" />
          </NavLink>
        </div>
      </section>
