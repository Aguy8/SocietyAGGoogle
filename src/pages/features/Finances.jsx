    ...
    <div className="page">
      ...
      <section data-nav-theme="dark" style={{ padding: '120px 64px 80px', background: 'var(--encre)' }}>
        <div className="container">
          ...
          <div style={{ display: 'inline-flex', padding: '8px 16px', background: 'var(--terre)22', border: '1px solid var(--terre)44', borderRadius: 999, marginBottom: 24 }}>
            <Icon name="money" size={18} color='var(--hibiscus)' />
            <span style={{ color: 'var(--cobalt)', fontWeight: 700, fontSize: 13, marginLeft: 8 }}>MODULE 02</span>
          </div>
          <h1 style={{ fontSize: 88, lineHeight: 0.9, color: 'white', maxWidth: 900 }}>
            Monétisation<br /><span style={{ color: 'var(--cobalt)' }}>& Finances.</span>
          </h1>
      ...
      <section style={{ padding: '80px 64px' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
            {[
              { icon: 'money', color: 'var(--hibiscus)', bg: '#FFFFFF', t: 'Cagnottes en 3 clics', d: 'Lancez une collecte en moins de 2 minutes. Lien partageable, suivi en temps réel, notifications automatiques.' },
              { icon: 'calendar', color: 'var(--cobalt)', bg: '#E8EFFC', t: 'Cotisations automatisées', d: 'Relances SMS, rappels par email, historique — plus aucun conflit de trésorerie.' },
              ...
              { icon: 'chart', color: 'var(--cobalt)', bg: '#F3F0FF', t: 'Crowdfunding participatif', d: 'Projets à financer avec objectif, durée, paliers de récompenses pour les donateurs.' },
              { icon: 'shield', color: 'var(--encre)', bg: '#F2F2F2', t: 'Reporting financier', d: 'Exports comptables, reçus automatiques, transparence totale pour les donateurs.' },
            ].map((f, i) => (
      ...
      <section style={{ padding: '80px 64px', background: 'var(--hibiscus)' }}>
        <div className="container">
          <Reveal style={{ textAlign: 'center', marginBottom: 48 }}>
            <h2 style={{ fontSize: 48, color: 'var(--encre)', fontWeight: 800 }}>Compatible avec tous vos paiements.</h2>
          </Reveal>
          <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            {['Orange Money', 'Wave', 'MTN MoMo', 'Flooz', 'Visa / Mastercard', 'Virement bancaire'].map((p, i) => (
              <div key={i} style={{ padding: '14px 24px', background: 'rgba(0,0,0,0.1)', borderRadius: 14, fontSize: 15, fontWeight: 700, color: 'var(--encre)' }}>{p}</div>
            ))}
          </div>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            <Reveal direction="left">
              ...
              <div style={{ display: 'grid', gap: 16, marginTop: 40 }}>
                {[
                  { n: '28 → 4 %', l: 'Taux d\'impayés estimé avec Society (vs. gestion manuelle)', color: 'var(--cobalt)' },
                  { n: '6h → 30min', l: 'Gain de temps estimé sur la comptabilité hebdomadaire', color: 'var(--hibiscus)' },
                  { n: '0', l: 'Conflit financier possible grâce à la traçabilité intégrale', color: 'var(--acacia)' },
                ].map((s, i) => (
            ...
            <Reveal direction="right">
              <div style={{ background: 'white', borderRadius: 28, padding: 32, boxShadow: '0 20px 60px rgba(0,0,0,0.08)', border: '1px solid #E5E5E5' }}>
                ...
                <div style={{ height: 8, background: '#F2F2F2', borderRadius: 999, marginTop: 8, overflow: 'hidden' }}>
                  <div style={{ width: '72%', height: '100%', background: 'linear-gradient(90deg, var(--cobalt), #3B7FFF)' }} />
                </div>
