...
export default function RSE() {
  ...
  return (
    <div className="page">
      ...
      <section data-nav-theme="dark" style={{ padding: '80px 64px', background: '#1A2BFF', color: 'white' }}>
      ...
      <section style={{ padding: '100px 64px' }}>
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
            {[
              { n: '01', icon: 'check', color: '#1A2BFF', t: 'Définissez vos objectifs', d: 'ODD ciblés, zones, types de communautés, budget. On construit le programme sur mesure.' },
              { n: '02', icon: 'users', color: '#1A2BFF', t: 'On connecte les communautés', d: 'Society identifie et vous présente les communautés alignées avec votre projet RSE.' },
              { n: '03', icon: 'chart', color: '#14110D', t: 'Mesurez l\'impact réel', d: 'Rapports mensuels automatiques. Bénéficiaires touchés, fonds utilisés, indicateurs ODD, témoignages terrain.' },
            ].map((s, i) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
            {OFFRES.map((o, i) => (
              <Reveal key={i} delay={i * 100}>
                <div className="hover-lift" style={{ padding: 36, background: o.featured ? '#1A2BFF' : 'white', borderRadius: 28, border: o.featured ? 'none' : '1px solid #E5E5E5', position: 'relative' }}>
                  {o.featured && <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', padding: '4px 16px', background: 'white', borderRadius: 999, fontSize: 11, fontWeight: 800, color: '#1A2BFF', whiteSpace: 'nowrap' }}>LE PLUS POPULAIRE</div>}
                  <div style={{ fontSize: 18, fontWeight: 800, color: o.featured ? 'white' : '#14110D', marginBottom: 8 }}>{o.t}</div>
                  ...
                  <a href="#contact-rse" className={o.featured ? 'btn' : 'btn btn-outline'} style={{ marginTop: 32, width: '100%', justifyContent: 'center', display: 'flex', background: o.featured ? 'white' : undefined, color: o.featured ? '#1A2BFF' : undefined }}>
                    Nous contacter
                  </a>
                </div>
              </Reveal>
      ...
      <section id="contact-rse" data-nav-theme="dark" style={{ padding: '100px 64px 120px', background: '#1A2BFF' }}>
            ...
            {sent ? (
              ...
              <div style={{ display: 'grid', gap: 14, marginTop: 48, textAlign: 'left' }}>
                ...
                <button
                  ...
                  className="btn"
                  style={{ width: '100%', justifyContent: 'center', background: rseForm.entreprise && rseForm.contact ? 'white' : 'rgba(255,255,255,0.3)', color: '#1A2BFF', fontWeight: 700, cursor: rseForm.entreprise && rseForm.contact ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}
                >
                  Envoyer ma demande <Icon name="arrow" size={16} color="#1A2BFF" />
                </button>
              </div>
            )}
          </Reveal>
