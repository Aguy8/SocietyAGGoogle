...
const COMMUNITY_EVENTS = [
  { icon: 'users', nom: 'Forums B2B & Salons', desc: 'Matchmaking inter-entreprises, délégations sectorielles, networking structuré.', color: 'var(--encre)' },
  { icon: 'globe', nom: 'Festivals & Divertissement', desc: 'Concerts, festivals culturels, événements artistiques — billetterie grand public.', color: 'var(--encre)' },
  { icon: 'layers', nom: 'Congrès & Conférences', desc: 'Événements académiques, remises de prix, keynotes — accréditations digitales.', color: 'var(--encre)' },
  { icon: 'award', nom: 'Galas & Cérémonies', desc: 'Dîners de prestige, remises de distinctions, soirées de levée de fonds.', color: 'var(--encre)' },
  { icon: 'file-text', nom: 'Assemblées générales', desc: 'AG ordinaires et extraordinaires, votes en ligne, quorum automatique.', color: 'var(--encre)' },
  { icon: 'calendar', nom: 'Événements culturels & sportifs', desc: 'Tournois, compétitions, spectacles — gestion des inscriptions et des accès.', color: 'var(--encre)' },
]
  ...
  return (
    <div className="page">
      ...
      <section style={{ padding: '120px 64px 80px', background: '#F8F8F8', position: 'relative', overflow: 'hidden', minHeight: '65vh', display: 'flex', alignItems: 'center' }}>
        <div style={{ position: 'absolute', top: -150, right: -150, width: 600, height: 600, borderRadius: '50%', background: 'var(--cobalt)', opacity: 0.05, filter: 'blur(120px)' }} />
        <div style={{ position: 'absolute', bottom: -80, left: -80, width: 400, height: 400, borderRadius: '50%', background: 'var(--cobalt)', opacity: 0.08, filter: 'blur(80px)' }} />
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          <Reveal>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '8px 20px', background: 'white', border: '1px solid #E5E5E5', borderRadius: 999, marginBottom: 32 }}>
              <Icon name="ticket" size={16} color='var(--cobalt)' />
              <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--cobalt)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Billetterie & Événements</span>
            </div>
            <h1 style={{ fontSize: 'clamp(56px, 8vw, 112px)', lineHeight: 0.88, color: 'var(--encre)', maxWidth: 900 }}>
              De la petite AG<br />
              au grand <span style={{ color: 'var(--cobalt)' }}>festival national.</span>
            </h1>
      ...
      <section style={{ padding: '100px 64px', background: '#FFFFFF' }}>
        <div className="container">
          <Reveal>
            ...
            <h2 style={{ fontSize: 'clamp(36px, 5vw, 64px)', color: 'var(--encre)', maxWidth: 700, lineHeight: 0.95, marginTop: 20 }}>
              Votre communauté,<br />
              <span style={{ color: 'var(--cobalt)' }}>vos événements.</span>
            </h2>
              ...
              <Reveal key={i} delay={i * 40}>
                <div style={{ padding: '28px', background: '#F8F8F8', borderRadius: 20, position: 'relative', overflow: 'hidden', border: '1px solid #EDE8DD' }}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, background: ev.color + '18', display: 'grid', placeItems: 'center', marginBottom: 16 }}>
                    <Icon name={ev.icon} size={22} color={ev.color || 'var(--cobalt)'} />
                  </div>
                  <h3 style={{ fontSize: 16, fontWeight: 800, color: 'var(--encre)', marginBottom: 8, lineHeight: 1.2 }}>{ev.nom}</h3>
          ...
          <div className="two-col-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
            ...
            <Reveal>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 14px', background: '#E8EFFC', border: '1px solid rgba(26,43,255,0.2)', borderRadius: 999, marginBottom: 24 }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--cobalt)' }} />
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--cobalt)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>La différence Society</span>
              </div>
              <h2 style={{ fontSize: 'clamp(32px, 4vw, 52px)', color: 'var(--encre)', lineHeight: 1, marginBottom: 24 }}>
                Chaque événement crée<br />
                sa propre <span style={{ color: 'var(--cobalt)' }}>communauté.</span>
              </h2>
              ...
              <p style={{ fontSize: 17, color: '#4A4438', lineHeight: 1.7 }}>
                Ce n'est pas juste de la billetterie. C'est un <strong style={{ color: 'var(--encre)' }}>mouvement autour de votre événement</strong> — contrôlé, mesuré, monétisable.
              </p>
            </Reveal>
            ...
            <Reveal delay={100}>
              <div style={{ background: 'white', borderRadius: 24, overflow: 'hidden', border: '1px solid #EBEBEB', boxShadow: '0 12px 40px rgba(0,0,0,0.06)' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid #F0F0F0', display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--encre)', display: 'grid', placeItems: 'center', flexShrink: 0 }}><Icon name="calendar" size={20} color="white" /></div>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--encre)' }}>Forum B2B Côte d'Ivoire 2026</div>
                    <div style={{ fontSize: 12, color: '#6B6B6B', marginTop: 2 }}>Communauté événement · 847 membres</div>
                  </div>
                  <div style={{ marginLeft: 'auto', padding: '4px 10px', background: '#E8EFFC', borderRadius: 999, fontSize: 11, fontWeight: 700, color: 'var(--cobalt)' }}>● Live</div>
                </div>
                <div style={{ padding: '18px 24px', borderBottom: '1px solid #F0F0F0' }}>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--encre)', display: 'grid', placeItems: 'center', flexShrink: 0 }}><Icon name="bell" size={14} color="white" /></div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--encre)', marginBottom: 4 }}>Organisateur · Teaser #3</div>
                ...
                <div style={{ padding: '14px 24px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                  {[['ticket', '847', 'Inscrits'], ['eye', '2 400', 'Vues/j'], ['bell', '94 %', 'Notifiés']].map(([e, v, l]) => (
                    <div key={l} style={{ textAlign: 'center', padding: '10px 8px', background: '#F8F8F8', borderRadius: 10 }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--encre)', marginTop: 4 }}>{v}</div>
                ...
                <div style={{ padding: '14px 24px 20px', borderTop: '1px solid #F0F0F0' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {['VIP · 120 places', 'Standard · 500 places', 'Presse · Sur demande'].map((t, i) => (
                      <span key={i} style={{ padding: '4px 10px', background: i === 0 ? '#E8EFFC' : '#F2F2F2', borderRadius: 999, fontSize: 11, fontWeight: 600, color: i === 0 ? 'var(--cobalt)' : '#6B6B6B' }}>{t}</span>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {COMMUNITY_FEATURES.map((f, i) => (
              <Reveal key={i} delay={i * 60}>
                <div style={{ padding: '24px', background: 'white', border: '1px solid #EBEBEB', borderRadius: 16, display: 'flex', gap: 16 }}>
                  <div style={{ flexShrink: 0 }}><Icon name={f.icon} size={20} color='var(--encre)' /></div>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 800, color: 'var(--encre)', marginBottom: 6 }}>{f.title}</div>
      ...
      <section style={{ padding: '100px 64px', background: 'white' }}>
        <div className="container">
          <Reveal>
            ...
            <h2 style={{ fontSize: 'clamp(36px, 5vw, 64px)', marginTop: 24, color: 'var(--encre)', lineHeight: 0.95 }}>
              De l'idée à l'événement<br />
              <span style={{ color: 'var(--encre)' }}>en 4 étapes.</span>
            </h2>
          </Reveal>
            ...
            {STEPS.map((step, i) => (
              <Reveal key={i} delay={i * 80}>
                <div style={{ padding: '40px 28px', borderRight: i < 3 ? '1px solid #EBEBEB' : 'none', textAlign: 'center', background: 'white' }}>
                  <div style={{ marginBottom: 16 }}><Icon name={step.icon} size={28} color='var(--encre)' /></div>
                  <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--encre)', letterSpacing: '0.1em', marginBottom: 12 }}>{step.num}</div>
                  <h3 style={{ fontSize: 16, fontWeight: 800, color: 'var(--encre)', marginBottom: 10 }}>{step.title}</h3>
      ...
      <section style={{ padding: '100px 64px', background: '#F8F8F8' }}>
        <div className="container">
          <Reveal>
            ...
            <h2 style={{ fontSize: 'clamp(36px, 5vw, 64px)', color: 'var(--encre)', maxWidth: 750, lineHeight: 0.95, marginTop: 20 }}>
              Society organise aussi<br />
              ses <span style={{ color: 'var(--cobalt)' }}>propres événements.</span>
            </h2>
                ...
                <div style={{ padding: '36px 40px', background: 'white', display: 'grid', gridTemplateColumns: '80px 1fr', gap: 32, alignItems: 'center', borderBottom: i < SOCIETY_INITIATIVES.length - 1 ? '1px solid #F0F0F0' : 'none' }}>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ width: 56, height: 56, borderRadius: 16, background: '#E8EFFC', display: 'grid', placeItems: 'center', margin: '0 auto 8px' }}>
                      <Icon name={ev.icon} size={24} color='var(--encre)' />
                    </div>
                    <div style={{ fontSize: 11, fontWeight: 800, color: '#9B9B9B', letterSpacing: '0.08em' }}>0{i + 1}</div>
                  </div>
                  <div>
                    <h3 style={{ fontSize: 22, fontWeight: 800, color: 'var(--encre)', marginBottom: 10, lineHeight: 1.1 }}>{ev.nom}</h3>
          ...
          <Reveal>
            <div style={{ marginTop: 32, padding: '20px 28px', background: 'white', border: '1px solid #EBEBEB', borderRadius: 16, display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
              <div><Icon name="users" size={18} color='var(--cobalt)' /></div>
      ...
      {/* CTA */}
      <section data-nav-theme="dark" style={{ padding: '120px 64px', background: 'var(--cobalt)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -100, right: -100, width: 500, height: 500, borderRadius: '50%', background: 'var(--encre)', opacity: 0.08 }} />
