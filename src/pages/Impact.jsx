...
const STATS = [
  { value: '25+', label: 'Leaders interviewés', sub: 'avant la première ligne de code', color: '#1A2BFF' },
  { value: '14', label: 'Pays africains ciblés', sub: 'dès le lancement en Afrique de l\'Ouest', color: '#1A2BFF' },
  { value: '6', label: 'Types de communautés', sub: 'dans notre panel de validation terrain', color: '#14110D' },
  { value: '850M', label: 'FCFA mobilisables/an', sub: 'estimation basée sur nos études de cas', color: '#1A2BFF' },
]

const SECTEURS = [
  { nom: 'ONG & Associations', pct: 34, color: '#1A2BFF', count: '~34 % des demandes' },
  { nom: 'Mutuelles & Tontines', pct: 28, color: '#14110D', count: '~28 % des demandes' },
  { nom: 'Alumni & Jeunesse', pct: 18, color: '#1A2BFF', count: '~18 % des demandes' },
  { nom: 'Leaders & Influenceurs', pct: 11, color: '#14110D', count: '~11 % des demandes' },
  { nom: 'Groupes religieux', pct: 6, color: '#1A2BFF', count: '~6 % des demandes' },
  { nom: 'Réseaux professionnels', pct: 3, color: '#1A2BFF', count: '~3 % des demandes' },
]
  ...
  return (
    <div className="page">
      ...
      <section style={{ padding: '120px 64px 80px', background: '#F8F8F8', position: 'relative', overflow: 'hidden', minHeight: '70vh', display: 'flex', alignItems: 'center' }}>
        <div style={{ position: 'absolute', top: -200, left: -200, width: 700, height: 700, borderRadius: '50%', background: '#1A2BFF', opacity: 0.08, filter: 'blur(120px)' }} />
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          <Reveal>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '8px 20px', background: 'rgba(26,43,255,0.08)', border: '1px solid rgba(26,43,255,0.20)', borderRadius: 999, marginBottom: 32 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: '#1A2BFF', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Impact en temps réel</span>
            </div>
            <h1 style={{ fontSize: 'clamp(56px, 8vw, 112px)', lineHeight: 0.88, color: '#14110D', maxWidth: 900 }}>
              L'Afrique<br />
              <span style={{ color: '#1A2BFF' }}>va s'organiser.</span><br />
              Ensemble.
            </h1>
            ...
            {STATS.map((s, i) => (
              <Reveal key={i} delay={i * 100}>
                <div style={{ padding: '48px 32px', textAlign: 'center', borderRight: i < 3 ? '1px solid #F0F0F0' : 'none' }}>
                  <div style={{ fontSize: 'clamp(36px, 5vw, 60px)', fontWeight: 900, color: '#1A2BFF', letterSpacing: '-0.03em', lineHeight: 1 }}>{s.value}</div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: '#14110D', marginTop: 12 }}>{s.label}</div>
      ...
      <section style={{ padding: '120px 64px', background: 'white' }}>
        <div className="container">
          <Reveal>
            ...
            <h2 style={{ fontSize: 'clamp(40px, 5vw, 72px)', marginTop: 24, color: '#14110D', maxWidth: 700, lineHeight: 0.95 }}>
              Des résultats mesurables,<br />
              <span style={{ color: '#1A2BFF' }}>à votre portée.</span>
            </h2>
          </Reveal>
            ...
            {IMPACT_CARDS.map((card, i) => (
              <Reveal key={i} delay={i * 80}>
                <div style={{ padding: '40px', background: '#F8F8F8', border: '1px solid #EBEBEB', borderRadius: 24, position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: '#1A2BFF' }} />
                  <div style={{ marginBottom: 20 }}><Icon name={card.icon} size={32} color="#1A2BFF" /></div>
                  <h3 style={{ fontSize: 22, fontWeight: 800, color: '#14110D', marginBottom: 12 }}>{card.title}</h3>
                  ...
                  <div style={{ padding: '16px 20px', background: 'white', border: '1px solid #E5E5E5', borderRadius: 12, display: 'flex', alignItems: 'baseline', gap: 10 }}>
                    <span style={{ fontSize: 28, fontWeight: 900, color: '#1A2BFF' }}>{card.metric}</span>
        ...
        <div className="container">
          <div className="two-col-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
            <Reveal>
              ...
              <h2 style={{ fontSize: 'clamp(36px, 4vw, 56px)', marginTop: 24, color: '#14110D', lineHeight: 1 }}>
                7 types de<br />
                <span style={{ color: '#1A2BFF' }}>communautés.</span><br />
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                        <span style={{ fontSize: 14, fontWeight: 600, color: '#14110D' }}>{s.nom}</span>
                        <span style={{ fontSize: 12, color: '#9B9B9B' }}>{s.count}</span>
                      </div>
                      <div style={{ height: 8, borderRadius: 99, background: '#F2F2F2', overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${s.pct}%`, background: '#1A2BFF', borderRadius: 99 }} />
                      </div>
                    </div>
                    <span style={{ fontSize: 13, fontWeight: 800, color: '#1A2BFF', minWidth: 36, textAlign: 'right' }}>{s.pct}%</span>
                  </div>
                ))}
              </div>
            </Reveal>
        ...
        <div className="container">
          <Reveal>
            <div style={{ padding: '28px 36px', background: 'white', border: '1px solid #EBEBEB', borderRadius: 20, display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap', boxShadow: '0 4px 16px rgba(0,0,0,0.04)' }}>
              <div style={{ flexShrink: 0, marginTop: 2 }}><Icon name="bell" size={28} color="#1A2BFF" /></div>
              <div style={{ flex: 1, minWidth: 260 }}>
                <div style={{ fontSize: 12, fontWeight: 800, color: '#1A2BFF', marginBottom: 8, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Notre engagement</div>
      ...
      <section style={{ padding: '100px 64px', background: 'white' }}>
        <div className="container">
          <Reveal>
            ...
            <h2 style={{ fontSize: 'clamp(36px, 4vw, 56px)', marginTop: 24, color: '#14110D', lineHeight: 1 }}>
              De la vision<br />
              <span style={{ color: '#1A2BFF' }}>au déploiement.</span>
            </h2>
          </Reveal>
              ...
              {MILESTONES.map((m, i) => (
                <Reveal key={i} delay={i * 80}>
                  <div style={{ display: 'flex', gap: 40, paddingLeft: 60, paddingBottom: 48, position: 'relative', opacity: m.done ? 1 : 0.85 }}>
                    <div style={{ position: 'absolute', left: 12, top: 4, width: 18, height: 18, borderRadius: '50%', background: m.done ? '#1A2BFF' : '#E5E5E5', border: '3px solid white', boxShadow: `0 0 0 2px ${m.done ? '#1A2BFF' : '#D5D5D5'}` }} />
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: '#1A2BFF', letterSpacing: '0.06em' }}>{m.date}</span>
                        <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 999, background: '#E8EFFC', color: '#1A2BFF', letterSpacing: '0.04em' }}>{m.done ? '✓ RÉALISÉ' : '◦ OBJECTIF'}</span>
                      </div>
                      <div style={{ fontSize: 18, fontWeight: 800, color: '#14110D', marginBottom: 8 }}>{m.label}</div>
      ...
      {/* CTA */}
      <section data-nav-theme="dark" style={{ padding: '120px 64px', background: 'linear-gradient(135deg, #1A2BFF 0%, #0B17B8 100%)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -100, right: -100, width: 500, height: 500, borderRadius: '50%', background: '#B22842', opacity: 0.1, filter: 'blur(80px)' }} />
