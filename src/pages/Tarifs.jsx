...
export default function Tarifs() {
  return (
    <div className="page">
        ...
        <div className="container">
          <Reveal>
            ...
            <h1 style={{ fontSize: 100, lineHeight: 0.9, maxWidth: 900, marginTop: 28 }}>
              Un prix juste,<br />
              <span style={{ color: '#1A2BFF' }}>conçu pour l'Afrique</span>.
            </h1>
            ...
            {plans.map((p, i) => (
              <Reveal key={i} delay={i * 80} direction="scale">
                <div style={{ padding: '36px 32px', background: p.color, color: p.text, borderRadius: 24, position: 'relative', display: 'flex', flexDirection: 'column', height: '100%', boxShadow: p.popular ? '0 24px 64px rgba(26,43,255,0.3)' : '0 4px 20px rgba(0,0,0,0.06)' }}>
                  {p.popular && (
                    <div style={{ position: 'absolute', top: -12, left: 24, padding: '5px 14px', background: '#F4ECD8', color: '#14110D', borderRadius: 999, fontSize: 10, fontWeight: 800, letterSpacing: '0.06em' }}>RECOMMANDÉ</div>
                  )}
                  ...
                  <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 10, flex: 1 }}>
                    {p.f.map((f, j) => (
                      <li key={j} style={{ display: 'flex', gap: 10, fontSize: 14, lineHeight: 1.4 }}>
                        <span style={{ flexShrink: 0, marginTop: 1, color: p.text === 'white' ? '#F4ECD8' : '#1A2BFF', fontWeight: 700 }}>✓</span>
                        <span style={{ fontWeight: j === 0 && f.includes('plus') ? 700 : 400 }}>{f}</span>
                      </li>
                    ))}
                  </ul>
                  {p.extra && (
                    <div style={{ marginTop: 12, padding: '8px 10px', background: p.text === 'white' ? 'rgba(255,255,255,0.08)' : 'rgba(14,71,171,0.06)', borderRadius: 8, fontSize: 11, color: p.text === 'white' ? 'rgba(255,255,255,0.55)' : '#1A2BFF' }}>
                      ↗ {p.extra}
                    </div>
                  )}
                  <NavLink to="/inscription" style={{ marginTop: 16, padding: '11px 16px', textAlign: 'center', borderRadius: 999, fontSize: 13, fontWeight: 700, textDecoration: 'none', background: p.text === 'white' ? 'white' : '#1A2BFF', color: p.text === 'white' ? (p.color === '#1A2BFF' ? '#1A2BFF' : p.color) : 'white', display: 'block' }}>{p.cta}</NavLink>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section data-nav-theme="dark" style={{ background: '#14110D', padding: '80px 64px' }}>
        <div className="container">
          ...
          <div style={{ marginTop: 32, padding: '24px 32px', background: 'rgba(212,167,91,0.12)', borderRadius: 16, border: '1px solid rgba(212,167,91,0.3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
            ...
            <NavLink to="/contact" style={{ padding: '12px 24px', background: '#F4ECD8', color: '#14110D', borderRadius: 999, fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>Contacter l'équipe →</NavLink>
          </div>
        </div>
      </section>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                {[
                  ...
                  <div key={i} style={{ padding: 24, background: 'white', borderRadius: 20, border: '1px solid #EDE8DD' }}>
                    <div style={{ width: 40, height: 40, borderRadius: 12, background: '#DDE0FF', display: 'grid', placeItems: 'center', marginBottom: 12 }}><Icon name={c.icon} size={18} color="#1A2BFF" /></div>
                    <div style={{ fontSize: 14, color: '#6B6B6B', fontWeight: 600 }}>{c.l}</div>
                    <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-0.02em', color: '#1A2BFF', marginTop: 4 }}>{c.v}</div>
        ...
        <div className="container">
          <div className="two-col-grid" style={{ gap: 80 }}>
            <Reveal direction="left">
              ...
              <p style={{ fontSize: 16, marginTop: 20, maxWidth: 360 }}>Une autre question ? <NavLink to="/contact" style={{ color: '#1A2BFF', fontWeight: 600 }}>Écrivez-nous →</NavLink></p>
            </Reveal>
            <div>
              {[
