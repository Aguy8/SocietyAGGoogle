...
const categories = [
  { icon: 'users', title: 'Gestion des membres', count: 12, color: '#E8F0FD', iconColor: '#1A2BFF' },
  { icon: 'money', title: 'Cotisations & paiements', count: 9, color: '#F8F8F8', iconColor: '#14110D' },
  { icon: 'calendar', title: 'Événements & billetterie', count: 7, color: '#F8F8F8', iconColor: '#1A2BFF' },
  { icon: 'money', title: 'Cagnottes & collectes', count: 6, color: '#E8EFFC', iconColor: '#1A2BFF' },
  { icon: 'link', title: 'API & intégrations', count: 11, color: '#E8EFFC', iconColor: '#1A2BFF' },
  { icon: 'shield', title: 'Compte & sécurité', count: 8, color: '#F8F8F8', iconColor: '#14110D' },
]
  ...
  return (
    <div className="page">
      ...
      <section data-nav-theme="dark" style={{ padding: '120px 64px 80px', background: '#1A2BFF', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2 }}>
          ...
          <h1 style={{ fontSize: 72, marginTop: 24, lineHeight: 0.95, color: 'white', maxWidth: 800 }}>
            ...
            <span style={{ color: '#1A2BFF' }}>vous aider ?</span>
          </h1>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40, flexWrap: 'wrap', gap: 16 }}>
            ...
            {search && (
              <button onClick={() => setSearch('')} style={{ padding: '10px 20px', background: 'white', border: '1px solid #E5E5E5', borderRadius: 12, fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}>
                <Icon name="x" size={14} color="#14110D" />
                Effacer
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {filtered.length === 0 && (
              <div style={{ padding: '48px', textAlign: 'center', background: 'white', borderRadius: 20 }}>
                ...
                <p style={{ fontSize: 14, color: '#9B9B9B', marginTop: 8 }}>Essayez d'autres mots-clés ou <NavLink to="/contact" style={{ color: '#1A2BFF', fontWeight: 600 }}>contactez notre support</NavLink>.</p>
              </div>
            )}
            {filtered.map((faq, i) => (
              <div key={i} style={{ background: 'white', borderRadius: 20, border: '1px solid #EBEBEB', overflow: 'hidden' }}>
                <button onClick={() => toggle(i)} style={{ width: '100%', padding: '24px 28px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 24, textAlign: 'left' }}>
                  <span style={{ fontSize: 16, fontWeight: 700, color: '#14110D', lineHeight: 1.4 }}>{faq.question}</span>
                  <div style={{ flexShrink: 0, width: 32, height: 32, borderRadius: 999, background: open === i ? '#1A2BFF' : '#F2F2F2', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name={open === i ? 'x' : 'arrow'} size={14} color={open === i ? 'white' : '#14110D'} />
                  </div>
                </button>
                {open === i && (
      ...
      <section style={{ padding: '64px 64px 120px' }}>
        <div className="container">
          <div style={{ background: '#14110D', borderRadius: 32, padding: '56px 64px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            <div>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              ...
              <div style={{ padding: '14px 24px', background: 'rgba(255,255,255,0.04)', borderRadius: 12, display: 'flex', gap: 16, alignItems: 'center', justifyContent: 'center' }}>
                <Icon name="calendar" size={16} color="#B22842" />
